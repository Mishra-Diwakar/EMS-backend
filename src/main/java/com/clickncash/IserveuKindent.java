...
public class IserveuKindent {

	private static final String CLIENT_ID = "u0Dau5m3STAzGimuSAMFDm72Ost76XiG3w5ofihjJYzOfv2f";
	private static final String CLIENT_SECRET = "2KZbHw6buTSgyn3uwtGbmQoI6Gg3tUkXGrEBzW4CK9A1gT75nHhvy6DSAHeZ1gPA";

	private static final String PAYOUT_CLIENT_ID = "u0Dau5m3STAzGimuSAMFDm72Ost76XiG3w5ofihjJYzOfv2f";
	private static final String PAYOUT_CLIENT_SECRET = "2KZbHw6buTSgyn3uwtGbmQoI6Gg3tUkXGrEBzW4CK9A1gT75nHhvy6DSAHeZ1gPA";
	
	private static final String CONTENT_TYPE = "application/json";
//	private static final String URL = "https://apiprod.iserveu.tech/productionV2/";
	...
	public static HashMap<String, Object> txnCheckPayInStatus(String requestData) {
		...
		try {
			...
			RequestBody body = RequestBody.create(mediaType, requestData);
			Request request = new Request.Builder()
					.url(URL+"statuscheck/txnreport").method("POST", body)
					.addHeader("client_id", CLIENT_ID)
					.addHeader("client_secret", CLIENT_SECRET)
					.addHeader("Content-Type", CONTENT_TYPE).build();
			Response response = client.newCall(request).execute();
	...
	public static String payout(String requestData) {
		...
		try {
			...
			RequestBody body = RequestBody.create(mediaType, requestData);
			Request request = new Request.Builder()
					.url(URL+"prod-apiusercashout/cashtransfer").method("POST", body)
					.addHeader("client_id", PAYOUT_CLIENT_ID)
					.addHeader("client_secret", PAYOUT_CLIENT_SECRET)
					.addHeader("Content-Type", CONTENT_TYPE).build();
			Response response = client.newCall(request).execute();
